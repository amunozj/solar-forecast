...
#        self.y_filepath = self.config["aia_path"] + "y/Y_GOES_XRAY_201401_201406_" + lag + "DELAY_" + catch + "MAX.csv"
        self.y_filepath = self.config["aia_path"] + "y/All_Ys_01hrDelay_12minMax.csv"
        ...
        self.y_prior_dict = {}
        self.y_prior_filepath = self.config["aia_path"] + "y/All_Ys_01hrDelay_12minMax.csv"
    ...
    def is_downloaded(self):
        ...
        if not os.path.isdir(self.config["aia_path"] + "y"):
            ...
            return False
        # if not os.path.isfile(self.config["aia_path"] + "y/All_Ys_01hrDelay_12minMax.csv"):
    ...
    def _get_flux(self, filename):
        ...
        """
        if len(filename) >38:
            inxUndr =  [m.start() for m in re.finditer('_', filename)]
            inxUndr = inxUndr[0]
            filename = filename[inxUndr+1:]
    ...
    def _get_aia_image(self, filename, directory, previous=0):
        ...
        #return feather.read_dataframe(directory + filename).values.reshape(1024,1024,8)
        # if previous == 0:
        #     return feather.read_dataframe(directory + filename).values.reshape(1024,1024,8)
        # while True:
        ...
        while True:
            ...
            previous -= 1
            if previous == 0:
                return feather.read_dataframe(directory + previous_filename).values.reshape(1024,1024,8)
