...
class AIA_HMI(dataset_models.dataset.Dataset):
    """
    ...
    def _get_aia_hmi_stack(self, filename, directory, previous=0):
        """
        ...
        if previous == 0:
            with warnings.catch_warnings():
                ...
                FN_dat = fits.open(directory + filename, cache=False)
                #print(directory + filename)
            return self._process_aia_hmi_stack(FN_dat)
        else:
    ...
    def _process_aia_hmi_stack(self, fitsStack):
        ...
        ## Process AIA stack
        for i in range(0,8):
            data[:,:,i] = fitsStack[i].data

        #Remove noise level (15 counts)
        data = data-15
        data[data<1] = 1
        np.log(data, out = data)

        ## Process HMI Magnetogram
        data[:,:,8] = fitsStack[8].data
        #Remove noise level (+/- 15 Gauss)
        data[np.logical_and(data[:,:,8]<15, data[:,:,8]>-15),8] = 0
        #Re-adjust zero
        data[data[:,:,8]>15,8] = data[data[:,:,8]>15,8]-15
        data[data[:,:,8]<-15,8] = data[data[:,:,8]<-15,8]+15
        #Apply inverse hyperbolic sine transform
        np.arcsinh(data[:,:,8], out = data[:,:,8])
    ...
    def _get_x_data(self, filename, training=None):
        ...
        for index in range(0, self.aia_image_count):
            current_data.append(self._get_aia_hmi_stack(filename, directory, previous=index))
